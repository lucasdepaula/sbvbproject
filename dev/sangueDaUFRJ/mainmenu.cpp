...
void MainMenu::createDatabase(){
    ...
    QSqlQuery qry2("CREATE TABLE BloodDrive (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT);",mydb);
    QSqlQuery qry3("CREATE TABLE DonationTime (id INTEGER, scheduledDate TEXT, scheduledTime TEXT, bloodDriveID INTEGER, maxDonors INTEGER DEFAULT(0), PRIMARY KEY (id, scheduledDate, scheduledTime));",mydb);
    QSqlQuery qry4("CREATE TABLE DonorSchedule (id INTEGER NOT NULL, donationTimeID INTEGER NOT NULL, donorID INTEGER NOT NULL, PRIMARY KEY (id, donationTimeID, donorID));",mydb);
